...
//----------------------------------Auction-----------------------------------------
    ...
    function bid(uint _itemId, uint wei_service_fee) external payable {
        require(itemTime[_itemId].started, "Auction not started");
        require(block.timestamp<=itemTime[_itemId].endAt,"Auction is over");
        ...
        uint lastMinCal = itemTime[_itemId].endAt - block.timestamp;
